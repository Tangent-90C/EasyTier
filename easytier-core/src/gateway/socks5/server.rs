use super::codec::{
    AuthenticationMethod, ReplyError, Result, Socks5Command, SocksError, TargetAddr, consts,
    read_address,
};
use anyhow::Context;
use std::io;
...
#[derive(Clone)]
pub(crate) struct Config<A: Authentication = DenyAuthentication> {
...
pub(crate) struct DenyAuthentication {}
...
#[derive(Copy, Clone, Default)]
pub(crate) struct AcceptAuthentication {}
...
#[async_trait::async_trait]
pub(crate) trait AsyncTcpConnector {
...
#[async_trait::async_trait]
pub(crate) trait Socks5UdpAssociation: Send {
...
#[async_trait::async_trait]
pub(crate) trait Socks5ServerRuntime: Send + Sync {
